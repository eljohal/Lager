...
public class SceneGeneralSettingsPostitionController{
    ...
    void initialize() {
        ...
        hersteller.setOnEditCommit(event -> {
        	...
        	//carManufactor.add(new Settings(z, ""));
            	...
            	if(value.equals("")) {
            		//delete
            		MySQLDatenbankConnection.update("DELETE FROM `möglichepositionen` WHERE `PID` = '"+y+"'");
            		...
            		if(pos == 0) {
            			if(!possibleSuggestionsModell.contains(value)) {
	            			max = MySQLDatenbankConnection.getInt("SELECT MIN(d1.PID)+1 FROM `möglichepositionen` d1 LEFT JOIN `möglichepositionen` d2 ON d1.PID + 1 = d2.PID WHERE d2.PID is NULL");
	            			
	            			MySQLDatenbankConnection.update("INSERT INTO `möglichepositionen`(`PID`, `Position`) VALUES ('"+max+"','"+value+"')");
            			...
            			if(!possibleSuggestionsModell.contains(value)) {
	            			MySQLDatenbankConnection.update("UPDATE `möglichepositionen` SET `Position`='"+value+"' WHERE `PID`='"+y+"'");
