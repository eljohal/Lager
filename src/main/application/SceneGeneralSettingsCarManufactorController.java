...
public class SceneGeneralSettingsCarManufactorController{
	...
	public ObservableList<Settings> getCarData(){
		...
		int id;
		z = 0;
		i_max = MySQLDatenbankConnection.getInt("SELECT MAX(MID) FROM `carmanufactor`");

		carManufactor.add(new Settings(0, ""));
		for(int i = 1;i <= i_max; i++) {
			id = MySQLDatenbankConnection.getInt("SELECT `MID` FROM `carmanufactor` WHERE `MID` = '" + i + "'");
			String value = MySQLDatenbankConnection.getString("SELECT `Manufactor` FROM `carmanufactor` WHERE `MID` = '" + i + "'");
			if(id != 0 || i == 0) {
    ...
    void initialize() {
        ...
        hersteller.setOnEditCommit(event -> {
            ...
            if(value.equals("")) {
            	//delete
            	MySQLDatenbankConnection.update("DELETE FROM `carmanufactor` WHERE `MID` = '"+y+"'");
            	...
            	if(pos == 0) {
            		if(!possibleSuggestionsModell.contains(value)) {
	           			max = MySQLDatenbankConnection.getInt("SELECT MIN(d1.MID)+1 FROM `carmanufactor` d1 LEFT JOIN `carmanufactor` d2 ON d1.MID + 1 = d2.MID WHERE d2.MID is NULL");
	          			MySQLDatenbankConnection.update("INSERT INTO `carmanufactor`(`MID`, `Manufactor`) VALUES ('"+max+"','"+value+"')");
            		...
            		if(!possibleSuggestionsModell.contains(value)) {
	           			MySQLDatenbankConnection.update("UPDATE `carmanufactor` SET `Manufactor`='"+value+"' WHERE `MID`='"+y+"'");
